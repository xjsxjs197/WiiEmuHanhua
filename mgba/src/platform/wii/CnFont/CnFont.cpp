...
*/
void InitCnFont() {
	int searchLen = (int)(mGba_CnFont_IA8_dat_size / (CHAR_IMG_SIZE + 4));
	int bufIndex = 0;
	int skipSetp = (CHAR_IMG_SIZE + 4) / 2;

	uint16_t *zhFontBufTemp = (uint16_t *)mGba_CnFont_IA8_dat;
...
void DrawCnChar(int x, int y, uint32_t color, uint16_t glyph) {
	...
	GX_InitTexObj(&fontTexObj, (u8*)(charImgBufMap[glyph]), CH_FONT_WIDTH, CH_FONT_HEIGHT, GX_TF_IA8, GX_CLAMP, GX_CLAMP, GX_FALSE);
