...
static void DrawGameInfoScreen(const gameinfo *gi, const MD5VerifyState_t *md5)
{
	...
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*6,  "标题:    %s", gi->Name);
	...
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*8,  "区域:   %s",
		BI2regions[(gi->Flags & GIFLAG_REGION_MASK) >> 3]);

	const u8 discNumber = ((gi->Flags & GIFLAG_DISCNUMBER_MASK) >> 5);
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*9,  "版本: %02u", gi->Revision);
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*10, "盘 #:   %u", discNumber+1);
	...
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*11, "格式: ");
