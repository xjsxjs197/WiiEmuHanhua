	...
	*(vu32*)0x92FFFFC0 = isWiiVC; //cant be detected in IOS
	...
	if(argsboot == false)
	{
		...
		while((ncfg->Config & NIN_CFG_AUTO_BOOT) && i < 1000000) // wait for wiimote re-synch
		{
			if (i == 0) {
				PrintInfo();
				PrintFormat(DEFAULT_SIZE, BLACK, 320 - 90, MENU_POS_Y + 20*10, "B: 取消自动启动");
				GRRLIB_Render();
				ClearScreen();
			}
		...
		if (CurDICMD) {
			...
			switch (CurDICMD) {
				...
				case DIP_CMD_DVDR:
					...
					break;
				default:
					snprintf(unkdev, sizeof(unkdev), "未知 (CMD: 0x%02lX)", CurDICMD);
					device = unkdev;
					break;
			}
	...
	*(vu32*)0xD3003420 = 0x0DEA;
	while(1)
	{
		...
		if(argsboot == false)
		{
			...
			PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*6, "加载打过补丁的内核... %d", STATUS_LOADING);
			if(STATUS_LOADING == 0)
			{
			...
			if((STATUS_LOADING > 0 || abs(STATUS_LOADING) > 1) && STATUS_LOADING < 20)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*7, "ES_Init... 完成!");
			if(STATUS_LOADING == 2)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*8, "初始化存储设备...");
			if(abs(STATUS_LOADING) > 2 && abs(STATUS_LOADING) < 20)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*8, "初始化存储设备... 完成!");
			if(STATUS_LOADING == -2)
				PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, MENU_POS_Y + 20*8, "初始化存储设备... 错误! %d  关闭中", STATUS_ERROR);
			if(STATUS_LOADING == 3)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*9, "加载USB/SD设置...");
			if(abs(STATUS_LOADING) > 3 && abs(STATUS_LOADING) < 20)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*9, "加载USB/SD设置... 完成!");
			if(STATUS_LOADING == -3)
	...
	/* 			if (timeout == 0)
				...
				PrintFormat(DEFAULT_SIZE, (STATUS_ERROR == -7) ? MAROON:BLACK, MENU_POS_X, MENU_POS_Y + 20*10, (STATUS_ERROR == -7) ? "检查 FS... 超时!" : "检查 FS...");
			}
			if(abs(STATUS_LOADING) > 5 && abs(STATUS_LOADING) < 20)
			{
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*10, "检查 FS... 完成!");
			...
			if(abs(STATUS_LOADING) > 6 && abs(STATUS_LOADING) < 20)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*12, "ES_LoadModules... 完成!");
			if(STATUS_LOADING == -6)
				PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, MENU_POS_Y + 20*12, "ES_LoadModules... 错误! %d 关闭中", STATUS_ERROR);
			if(STATUS_LOADING == 7)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*13, "加载设定信息...");
			if(abs(STATUS_LOADING) > 7 && abs(STATUS_LOADING) < 20)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*13, "加载设定信息... 完成!");
			/*if(STATUS_LOADING == 8)
			{
			...
			if(STATUS_LOADING == 9)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*14, "初始化 DI... %40s", " ");
			if(abs(STATUS_LOADING) > 9 && abs(STATUS_LOADING) < 20)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*14, "初始化 DI... 完成! %35s", " ");
			if(STATUS_LOADING == 10)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*15, "初始化 CARD...");
			if(abs(STATUS_LOADING) > 10 && abs(STATUS_LOADING) < 20)
				PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*15, "初始化 CARD... 完成!");
	...
	if(argsboot == false)
	{
		...
		PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*17, "Nintendont内核运行中, 加载游戏中...");
		GRRLIB_Render();
		DrawBuffer(); // Draw all status messages
	}
