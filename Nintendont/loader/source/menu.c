...
void PrintInfo(void)
{
	...
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*2, "固件    : %u.%u.%u",
		    *(vu16*)0x80003140, *(vu8*)0x80003142, *(vu8*)0x80003143);
}
...
void PrintLoadKernelError(LoadKernelError_t iosErr, int err)
{
	...
	switch (iosErr)
	{
		...
		case LKERR_IOS_Open_shared1_content_map:
			...
			PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, MENU_POS_Y + 20*10, "请确保meta.xml文件和Nintendont存在同一目录，");
			PrintFormat(DEFAULT_SIZE, MAROON, MENU_POS_X, MENU_POS_Y + 20*11, "并且内容中必须包括<ahb_access/>标签.");
			break;
