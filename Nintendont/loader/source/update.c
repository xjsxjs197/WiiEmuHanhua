...
static const downloads_t Downloads[] = {
	{"https://raw.githubusercontent.com/FIX94/Nintendont/master/loader/loader.dol", "下载Nintendont主程序", "boot.dol", 0x400000}, // 4MB
	{"https://raw.githubusercontent.com/FIX94/Nintendont/master/nintendont/titles.txt", "下载列表文件titles.txt", "titles.txt", 0x80000}, // 512KB
	{"https://raw.githubusercontent.com/FIX94/Nintendont/master/controllerconfigs/controllers.zip", "下载手柄文件controllers.zip", "controllers.zip", 0x8000}, // 32KB
	{"https://raw.githubusercontent.com/FIX94/Nintendont/master/nintendont/gcn_md5.zip", "下载MD5文件gcn_md5.txt", "gcn_md5.zip", 0x20000}, // 128 KB
	{"https://raw.githubusercontent.com/FIX94/Nintendont/master/common/include/NintendontVersion.h", "检查版本信息", "", 0x400} // 1KB
};
...
static inline bool LatestVersion(int *major, int *minor, int *current_line) {
	...
	if(!http_request(Downloads[DOWNLOAD_VERSION].url, Downloads[DOWNLOAD_VERSION].max_size)) {
		PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*line, "检查版本失败");
		UpdateScreen();
		*current_line = line;
		return false;
	}
	...
	if ((*major <= NIN_MAJOR_VERSION) && (*minor <= NIN_MINOR_VERSION)) {
		...
		PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*line, "现在的版本就是最新的");
		line++;
		PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*line, "还要更新吗? (A: 是, B: 否)");
		line++;
		UpdateScreen();
		while(true) {
			DrawBuffer();
...
static s32 Download(DOWNLOADS download_number)  {
	...
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*line, "网络初始化完成");
	UpdateScreen();
	...
	if (download_number == DOWNLOAD_NINTENDONT) {
		...
		if (!ret) {
			PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*line, "下载取消了");
			UpdateScreen();
			if (outbuf != NULL) free(outbuf);
			net_deinit();
			sleep(4);
		...
		PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*line, "下载Nintendont主程序 v%i.%i", major, minor);
		UpdateScreen();
		line++;
	}
	...
	PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*line, "下载完成");
	UpdateScreen();
	line++;
	...
	if (ret == 1) {
		PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*line, "更新完成");
		UpdateScreen();
		line++;
	}
	...
	if (ret != 1) {
		...
		PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X, MENU_POS_Y + 20*line, "重新启动Nintendont来完成更新");
	}
	UpdateScreen();
	if (outbuf != NULL) free(outbuf);
	net_deinit();
...
void UpdateNintendont(void) {
	...
	while (true) {
		if (redraw) {
			...
			// Update menu.
			PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X + 50, MENU_POS_Y + 20*5, "下载Nintendont主程序");
			PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X + 50, MENU_POS_Y + 20*6, "下载列表文件titles.txt");
			PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X + 50, MENU_POS_Y + 20*7, "下载手柄文件controllers.zip");
			PrintFormat(DEFAULT_SIZE, BLACK, MENU_POS_X + 50, MENU_POS_Y + 20*8, "下载MD5文件gcn_md5.txt");
